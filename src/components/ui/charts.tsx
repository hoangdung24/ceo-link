...
function showTooltip(context: ChartContext) {
  ...
  el.className = `p-2 bg-card text-card-foreground  border shadow-sm text-sm ${
  ...
  body.forEach((line, i) => {
    ...
    content += `
        <div class="flex items-center">
          <span class="inline-block h-2 w-2 mr-1 border" style="background: ${colors.backgroundColor}; border-color: ${colors.borderColor}"></span>
          ${line}
        </div>`;
  });
  content += `</div>`;
